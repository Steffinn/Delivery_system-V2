...
**Delivery System** is a ground control station for ArduPilot-based vehicles (copters, planes,
rovers, boats). It is a rebrand of [Mission Planner](https://ardupilot.org/planner/) by Michael
Oborne / the ArduPilot Dev Team, with all the same features, packaged under a new name, icon and
color theme by Drobots Tech.

## Download & Install

1. Go to the [Releases](../../releases) page of this repository.
2. Download the latest `DeliverySystemSetup.exe`.
3. Run it and follow the install wizard (no admin rights required).
   - Windows SmartScreen may show an "Unknown publisher" warning since the installer isn't
     code-signed — click **More info → Run anyway** to continue.

## Uninstall

Use **Settings → Apps → Delivery System → Uninstall**, or run the "Uninstall Delivery System"
shortcut created in the Start Menu.

## License & attribution

This project is a derivative of Mission Planner (GPLv3), Copyright © Michael Oborne 2010-2024.
It remains licensed under the GPLv3 — see [COPYING.txt](COPYING.txt). All ArduPilot/MAVLink
protocol support and underlying functionality is unchanged from upstream; only branding
(name, icon, color theme) has been modified.

Upstream project: https://github.com/ArduPilot/MissionPlanner
